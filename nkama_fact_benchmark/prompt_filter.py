...
def prepare_execution_prompt(prompt: str, *, mode: str = "strict") -> str:
    ...
    return (
        ...
        f"{prompt.strip()}\n\n"
        "## 1. Intent Contract\n\n"
        "Before building, state this contract in your own words, one line per item, then build to it. Comprehension is not optional; a perfect answer to a task the user did not ask is a failure.\n\n"
        "- Goal: the single sentence the user most wants to be true when you finish.\n"
        "- Audience: who consumes the result, at what skill level.\n"
        "- Deliverable: the concrete artifact(s) the user receives.\n"
        "- Definition of done: the specific checks that, passing, mean the goal is met.\n"
        "- Out of scope: what you will not build. Anything outside the deliverable map is out of scope unless it blocks a deliverable.\n"
        "- Assumptions: what you decided because the user did not say, stated plainly so they can veto it.\n\n"
        "Every later section serves this contract. If a step stops serving the goal line, stop and re-read the contract instead of building momentum on the wrong road.\n\n"
        "## 2. Deliverable Map\n\n"
        f"{deliverables}\n\n"
        ...
        "```\n\n"
        "For executable proof, use argv-style command checks. Example:\n\n"
        "```json\n"
        "{\n"
        "  \"allowed_command_prefixes\": [[\"python3\"], [\"node\"]],\n"
        "  \"checks\": [\n"
        "    {\n"
        "      \"id\": \"tests_pass\",\n"
        ...
        "Tests or checks run:\n"
        "Intent check: restate the goal line from the Intent Contract and say plainly whether the delivered result serves it.\n"
...
def wrap_prompt(prompt: str, *, mode: str = "strict") -> str:
    ...
    return (
        ...
        "- Answer\n"
        "- Evidence\n"
        "- Intent check (one sentence: the user's goal, and whether the result serves it)\n"
        "- Limitations\n"
